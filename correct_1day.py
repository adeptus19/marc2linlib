...
def rec_sorted(record) :
    ...
    return result
def setupdate(peldany):
    vkpos = peldany[0].index("$5")
    vkod = peldany[1][vkpos]
    updatesql = "UPDATE kpld SET statusz = '3 nap' WHERE vonalkod ='" + vkod + " ';"
    print(updatesql)
    line = updatesql + "\n"
    outputf.write(line)
    return peldany
...
filestruct.close()
outputf = open("correct1day.sql","w", encoding="latin2")
try :
...
for line in fhandle :
    ...
    if len(line.strip()) > 0 :
        ...
        for pld in testrec["exempl"]:
            kolcspos = pld[0].index("$F")
            if pld[1][kolcspos] == "1 éjszaka":
                setupdate(pld)
fhandle.close()
outputf.close()